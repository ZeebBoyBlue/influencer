...
export function classifyInfluencerIntent(
  ...
  const exportTerms = ["export", "csv", "spreadsheet", "download", "save list"];

  const reasons: string[] = [];

  if (
    includesAny(request, exportTerms) &&
    (context.hasShortlist || context.hasCandidates)
  ) {
    reasons.push(
      "Request is asking to export an existing shortlist.",
    );
    return {
      step: "export",
      confidence: 0.9,
      reasons,
      suggestedNextAction:
        "Run the export helper script to produce CSV or JSON output.",
    };
  }
