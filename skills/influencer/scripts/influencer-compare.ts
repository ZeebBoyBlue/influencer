...
export function compareInfluencers(input: CompareInput): ComparedInfluencer[] {
  ...
  return scoreProfiles(profiles, criteria)
    ...
    .map((entry) => ({
      ...
      engagementRate: entry.profile.engagement?.engagementRate,
      engagementRateLabel: entry.profile.engagement?.engagementRate !== undefined
        ? `${entry.profile.engagement.engagementRate}%`
        : undefined,
      lastPostDaysAgo: entry.profile.lastPostDaysAgo,
      lastPostDate: entry.profile.engagement?.lastPostDate,
      postsPerWeek: entry.profile.engagement?.postsPerWeek,
      isDormant: Boolean(entry.profile.isDormant),
      suspiciousFlags: entry.profile.engagement?.suspiciousFlags ?? [],
      bioLinks: entry.profile.bioLinks ?? [],
      contentThemes: entry.profile.contentThemes ?? [],
    }));
}
